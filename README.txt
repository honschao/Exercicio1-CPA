Orientações

	1. Rode as células na ordem em que elas se apresentam.
	2. Utilize a versão 3.12 do python.
	3. Não é necessário abrir um servidor local.
	4. {IMPORTANTE} É necessário que o notebook esteja na mesma pasta que as outras pastas, caso contrário o script não funcionará.
	
============================================================================

Informações

	Após rodar todas as células do notebook os arquivos solicitados pelo professor estarão salvos nas pastas: Verbetes_htmls, Infoboxes_json e Infoboxes_json_testes.
		
============================================================================

Link para o repositório: https://github.com/honschao/Exercicio1-CPA


